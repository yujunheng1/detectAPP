...
Java_com_example_myapplication_yolo8_detectNative(JNIEnv *env, jobject thiz, jlong nativePtr, jobject image, jfloat threshold, jfloat nmsThreshold) {
    ...
    jclass objectClass = env->FindClass("com/example/myapplication/Object_box");
    jmethodID constructor = env->GetMethodID(objectClass, "<init>", "(FFFFFI)V");
...
// use qsort form https://github.com/nihui/ncnn-android-yolov5
void yolo8::qsort_descent_inplace(std::vector<Object_box>& dectObjects, int left, int right)
{
    int i = left;
    int j = right;
    float p = dectObjects[(left + right) / 2].score;

    while (i <= j)
    {
        while (dectObjects[i].score > p)
            i++;

        while (dectObjects[j].score < p)
            j--;

        if (i <= j)
        {
            // swap
            std::swap(dectObjects[i], dectObjects[j]);
...
#pragma omp section
        {
            if (left < j) qsort_descent_inplace(dectObjects, left, j);
        }
#pragma omp section
        {
            if (i < right) qsort_descent_inplace(dectObjects, i, right);
        }
    }
}
// scale the image into the targe size 640*640
...
//function from  https://github.com/nihui/ncnn-android-yolov5
...
void yolo8::generate_proposals(const std::vector<GridAndStride> grid_strides, const ncnn::Mat& pred,const float prob_threshold, std::vector<Object_box>& objects)
{
...
//use from  https://github.com/nihui/ncnn-android-yolov5
...
//use nms algorithm from  https://github.com/nihui/ncnn-android-yolov5
void yolo8::nms_sorted_bboxes(const std::vector<Object_box>& dectobjects, std::vector<int>& picked, float nms_threshold)
{
    picked.clear();

    const int n = dectobjects.size();

    std::vector<float> areas(n);
    for (int i = 0; i < n; i++)
    {
        areas[i] = dectobjects[i].w * dectobjects[i].h;
    }

    for (int i = 0; i < n; i++)
    {
        const Object_box& a = dectobjects[i];

        int keep = 1;
        for (int j = 0; j < (int)picked.size(); j++)
        {
            const Object_box& b = dectobjects[picked[j]];
